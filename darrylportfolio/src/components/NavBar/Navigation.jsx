        ...
        <Menu styles={styles} width={'100vw'} right isOpen={menuOpen} onStateChange={({ menuOpen }) => handleStateChange(menuOpen)}>
          ...
          <div className="w-full">
            <ul className='mobile-social flex justify-evenly items-center h-1/3'>
              {/* Linked In */}
              <a className="transition-transform-colors inline-block hover:bg-li-blue hover:-translate-y-1 duration-500 rounded-full" href="https://www.linkedin.com/in/darryltillman/" target='_blank' rel="noreferrer" name="Link to Darryl's LinkedIn"><img className="contactlink w-14" src={lisvg} alt="LinkedIn logo" /></a>
              {/* GitHub */}
              <a className="transition-transform-colors inline-block hover:bg-gh-grey hover:-translate-y-1 duration-500 rounded-full" href="https://github.com/Tales64" target='_blank' rel="noreferrer" name="Link to Darryl's GitHub Profile"><img className="contactlink w-14" src={ghsvg} alt="GitHub logo" /></a>
              {/* Gmail */}
              <a className="transition-transform-colors inline-block hover:bg-gm-red hover:-translate-y-1 duration-500 rounded-full" href="mailto:tales64@gmail.com" target='_blank' rel="noreferrer" name="Link to create and send an email."><img className="contactlink w-14" src={gmsvg} alt="Mail logo." /></a>
            </ul>
          </div>
        </Menu>
      </nav>
